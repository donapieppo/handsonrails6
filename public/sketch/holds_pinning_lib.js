export class Hold {
  ...
  draw(canvas, width) {
    ...
    const x0 = this.x - window.scrollX;
    const y0 = this.y - window.scrollY;
...
export class HoldPinner {
  constructor(target, img, width, height) {
    this.width = width;
    this.height = height;
  ...
  add_hold(e) {
    ...
    const x = e.x - this.canvas.getBoundingClientRect().left + window.scrollX;
    const y = e.y - this.canvas.getBoundingClientRect().top + window.scrollY;
